...
AS
(
SELECT dea.[continent], dea.[location], dea.[date], dea.[population], vac.[new_vaccinations], SUM(CONVERT(bigint,vac.[new_vaccinations])) OVER (PARTITION BY dea.[location] ORDER BY dea.[location],dea.[date]) AS [RunningTotalVaccinations]
FROM PortfolioProject2..CovidDeaths dea
JOIN PortfolioProject2..CovidVaccinations vac
	ON dea.[location] = vac.[location]
	AND dea.[date] = vac.[date]
